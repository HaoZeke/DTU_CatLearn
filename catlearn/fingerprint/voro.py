...
class VoronoiFingerprintGenerator(object):
    ...
    def __init__(self, atoms, delete_temp=True):
        ...
        self.cif_path = f'{self.temp_path}/cif/'
...
exit''' % (self.magpie_path, self.cif_path, 'id', self.temp_path)
    ...
    def write_voro_input(self):
        ...
        for i, at in enumerate(self.atoms):
            at_name = f'{str(i)}-' + at.get_chemical_formula()
            ...
            id_list.append(i)
        pro_dict = {'filename': filename, 'formula': fml, 'id': id_list}
        ...
        with open(self.temp_path + '/voro_FP.in', 'w') as f:
            f.writelines(self.voro_input)

    def run_voro(self):
        """Call Magpie to generate Voronoi FP and write to voro_FP.csv."""
        os.system(
            f"{self.magpie} {self.temp_path}/voro_FP.in |tee {self.temp_path}/voro_FP.log"
        )
