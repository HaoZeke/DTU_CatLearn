...
class Boundary_conditions:
    ...
    def create(self,TP,X,Y,parameters,log=True):
        ...
        if self.bound_type=='no':
            ...
            bounds=self.create_length(TP,X,Y,parameters)
        elif self.bound_type=='restricted':
            bounds=self.create_restricted(TP,X,Y,parameters)
        elif self.bound_type=='educated':
            bounds=self.create_educated(TP,X,Y,parameters)
    ...
    def create_restricted(self,TP,X,Y,parameters):
        ...
        for para in sorted(set(parameters)):
            if para=='length':
                bounds[para]=self.length_bound(X,Y,TP,scale=self.scale)
            elif para=='noise':
    ...
    def create_educated(self,TP,X,Y,parameters):
        ...
        for para in sorted(set(parameters)):
            if para=='length':
                ...
                if 'noise_deriv' in parameters and TP.use_derivatives:
                    bounds[para]=self.noise_bound(X,Y[:,0:1],scale=self.scale)
                else:
                    bounds[para]=self.noise_bound(X,Y,scale=self.scale)
            elif para=='noise_deriv' and TP.use_derivatives:
                bounds[para]=self.noise_bound(X,Y[:,1:],scale=self.scale)
            else:
                bounds[para]=np.array([[eps_mach_lower,1/eps_mach_lower]]*parameters.count(para))
        return bounds

    def length_bound(self,X,Y,TP,scale=1):
