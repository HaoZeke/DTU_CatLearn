...
def local(fun,x0,tp,parameters,X,Y,prior,dis_m,local_run=scipy_opt,maxiter=5000,jac=True,local_kwargs={},**global_kwargs):
    " Local optimization "
...
def local_prior(fun,x0,tp,parameters,X,Y,prior,dis_m,local_run=scipy_opt,maxiter=5000,jac=True,local_kwargs={},**global_kwargs):
    " Local optimization "
    args=(tp,parameters,X,Y,prior,jac,dis_m)
    sol=local_run(fun.function,x0,jac=jac,maxiter=maxiter,args=args,**local_kwargs)
    sol=fun.get_solution(sol,*args)
    if prior is not None:
        niter=sol['nfev']
        args=(tp,parameters,X,Y,None,jac,dis_m)
...
def local_ed_guess(fun,x0,tp,parameters,X,Y,prior,dis_m,local_run=scipy_opt,maxiter=5000,jac=True,local_kwargs={},**global_kwargs):
    " Local optimization for initial and educated guess hyperparameter sets "
    args=(tp,parameters,X,Y,prior,jac,dis_m)
    sol=local_run(fun.function,x0,jac=jac,maxiter=maxiter,args=args,**local_kwargs)
    from ..educated import Educated_guess
    hp_ed=Educated_guess(tp).hp(X,Y,parameters=parameters)
...
def random(fun,x0,tp,parameters,X,Y,prior,dis_m,local_run=scipy_opt,maxiter=5000,jac=True,bounds=None,npoints=50,use_bounds=True,local_kwargs={},**global_kwargs):
...
def grid(fun,x0,tp,parameters,X,Y,prior,dis_m,local_run=scipy_opt,maxiter=5000,jac=True,bounds=None,n_each_dim=None,optimize=True,use_bounds=True,local_kwargs={},**global_kwargs):
...
def line(fun,x0,tp,parameters,X,Y,prior,dis_m,local_run=scipy_opt,maxiter=5000,jac=True,bounds=None,n_each_dim=None,loops=3,optimize=True,use_bounds=True,local_kwargs={},**global_kwargs):
...
def basin(fun,x0,tp,parameters,X,Y,prior,dis_m,maxiter=5000,jac=True,niter=5,interval=10,T=1.0,stepsize=0.1,niter_success=None,local_kwargs={},**global_kwargs):
...
def annealling(fun,x0,gp,parameters,X,Y,prior,dis_m,maxiter=5000,jac=False,bounds=None,initial_temp=5230.0,restart_temp_ratio=2e-05,visit=2.62,accept=-5.0,seed=None,no_local_search=False,use_bounds=True,local_kwargs={},**global_kwargs):
    ...
    if bounds is None:
        ...
        sol=dual_annealing(anneal_var_trans,bounds=bounds,args=args,maxfun=maxiter,local_search_options=local_kwargs,**options_dual)        
    else:
        args=(gp,parameters,X,Y,prior,False,dis_m)
        sol=dual_annealing(fun.function,bounds=bounds,args=args,maxfun=maxiter,local_search_options=local_kwargs,**options_dual)
    sol=fun.get_solution(sol,gp,parameters,X,Y,prior,False,dis_m)
    return sol

def line_search_scale(fun,x0,tp,parameters,X,Y,prior,dis_m,local_run=run_golden,maxiter=5000,jac=False,bounds=None,ngrid=80,use_bounds=True,local_kwargs={},**global_kwargs):
